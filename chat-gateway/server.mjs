...
const ROLE_PROMPT_CACHE_MS = 30_000;
/** @type {{ at: number, sales: string, support: string }} */
let rolePromptCache = { at: 0, sales: "", support: "" };

async function fetchRolePromptsFromDb() {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    return { sales: "", support: "" };
  }
  const url = new URL("/rest/v1/chat_agent_role_prompts", SUPABASE_URL);
  url.searchParams.set("select", "role,system_prompt");
  const res = await fetch(url, {
    headers: {
      apikey: SUPABASE_SERVICE_ROLE_KEY,
      authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
  ...
  if (!res.ok) return { sales: "", support: "" };
  const rows = await res.json().catch(() => null);
  const out = { sales: "", support: "" };
  if (!Array.isArray(rows)) return out;
  for (const row of rows) {
    const role = String(row?.role || "").trim();
    const prompt = String(row?.system_prompt || "").trim();
    if (role === "sales" || role === "support") out[role] = prompt;
  }
  return out;
}

async function getRolePrompt(role) {
  const key = role === "sales" ? "sales" : "support";
  const now = Date.now();
  if (now - rolePromptCache.at > ROLE_PROMPT_CACHE_MS) {
    rolePromptCache = { at: now, ...(await fetchRolePromptsFromDb()) };
  }
  return rolePromptCache[key] || "";
}
