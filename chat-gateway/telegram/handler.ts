...
import { fallbackReply, systemPromptForRole, wrapKnowledgeBase } from "./prompt.js";
...
export function createTelegramAdapter(hooks: GatewayHooks, http: { text: HttpText; json: HttpJson }) {
  ...
  async function processUpdate(botId: string, req: IncomingMessage, body: TelegramUpdate): Promise<void> {
    ...
    let reply = "";
    try {
      const rag = await getRagContext(botId, query);
      ...
      const knowledge = wrapKnowledgeBase(formatKnowledgeBlock(rag.chunks), rag.degraded);
      reply = await completeChat(
        [
          { role: "system", content: systemPromptForRole(agent.bot_role) },
          { role: "system", content: knowledge },
          { role: "system", content: `Язык ответа: ${lang || agent.default_lang || "en"}.` },
          ...history,
