"""Live model catalogs for DeerFlow sync (copy of agent-api/swoop_provider_catalog.py)."""
...
_OPENROUTER_META_CACHE_TTL_SEC = 86400.0
...
def fetch_openrouter_catalog_meta(api_key: str = "") -> List[Dict[str, Any]]:
    ...
    return [{"id": mid, "created": created} for created, mid in ranked]
...
def fetch_openmodel_models(api_key: str, base_url: str) -> List[str]:
    key = (api_key or "").strip()
    origin = (base_url or "").strip().rstrip("/").removesuffix("/v1") or "https://api.openmodel.ai"
    if not key:
        return []
    code, body = _http_get_json(
        f"{origin}/v1/models",
    ...
    for item in body.get("data") or []:
        ...
        if mid:
            models.append(mid)
    models.sort(key=lambda m: (m.lower(), m))
...
def collect_live_provider_models(vals: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if glm_key:
        ...
        if live:
            out["glm"] = live
            logger.info("catalog: glm %s models (e.g. %s)", len(live), live[0])
    ...
    if openai_key:
        ...
        if live:
            out["openai"] = live
            logger.info("catalog: openai %s models (e.g. %s)", len(live), live[0])
    ...
    if lm_key:
        ...
        if live:
            out["lmarena"] = live
            logger.info("catalog: lmarena %s models (e.g. %s)", len(live), live[0])
    ...
    if mimo_key:
        ...
        if live:
            out["mimo"] = live
            logger.info("catalog: mimo %s models (e.g. %s)", len(live), live[0])
    ...
    if kimi_key:
        ...
        if live:
            out["kimi"] = live
            logger.info("catalog: kimi %s models (e.g. %s)", len(live), live[0])

    om_key = _first_key(vals.get("OPENMODEL"))
    if om_key:
        live = fetch_openmodel_models(om_key, str(vals.get("OPENMODEL_BASE") or ""))
        if live:
            out["openmodel"] = live
            logger.info("catalog: openmodel %s models (e.g. %s)", len(live), live[0])
