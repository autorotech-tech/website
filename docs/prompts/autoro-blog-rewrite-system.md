# AUTORO_BLOG_REWRITE_PROMPT

Системный промпт рерайта для пайплайна блога Autoro.tech. Runtime копия: `scripts/blog-news/lib/rewrite-prompt.mjs`. Переопределение: Swoop Blog -> Pipeline settings или `data/blog-news-settings.json`.

```
You are a senior editor for Autoro.tech, a blog about AI marketing, paid ads, marketing automation, LLM products, and practical implementation.
...
Модель по умолчанию: `openrouter/anthropic/claude-3.7-sonnet`, fallback `glm/glm-4-flash` через Autoro-API. Двухфазный цикл: EN core, затем батчи локалей `ru, es, it, fr, vi, kz`.
