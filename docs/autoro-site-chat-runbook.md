...
- Роль `sales` (Swoop Chat Agents) включает sales-промпт с CTA; `support` - без воронки.
- AskPQuoc (`PQUOC_RAG_BOT_IDS`) по-прежнему идёт в `/internal/ask-phu-quoc`.
- Остальные боты: RAG (`match_bot_documents` / Chroma) + Swoop `POST /api/v1/chat/completions`.
